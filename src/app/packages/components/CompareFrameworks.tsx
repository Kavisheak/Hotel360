    ...
    <section className="w-full bg-[#F0E6D0] py-20">
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        <div className="lg:col-span-8">
          <p className="text-[#A67C52] text-[10px] tracking-[0.2em] uppercase font-bold mb-3">
            Detailed Analysis
          </p>
          <h2 className="text-4xl md:text-5xl font-serif text-[#2C1E14] mb-10">
            Compare Frameworks
          </h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b-2 border-[#D4C9A8]">
                  <th className="py-4 pr-4 text-[10px] uppercase tracking-widest text-[#2C1E14] font-bold w-1/4">Feature / Service</th>
                  <th className="py-4 px-4 text-[10px] uppercase tracking-widest text-gray-500 font-bold w-1/4">Silver</th>
                  <th className="py-4 px-4 text-[10px] uppercase tracking-widest text-[#C9A84C] font-bold w-1/4">Gold</th>
                  <th className="py-4 pl-4 text-[10px] uppercase tracking-widest text-gray-500 font-bold w-1/4">Diamond</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#D4C9A8]/50">
                {compareData.map((row, idx) => (
                  <tr key={idx} className="hover:bg-white/40 transition-colors">
                    <td className="py-5 pr-4 text-xs font-bold text-[#2C1E14]">{row.feature}</td>
                    <td className="py-5 px-4 text-xs text-gray-600 font-medium leading-relaxed">{row.silver}</td>
                    <td className="py-5 px-4 text-xs text-[#A67C52] font-bold leading-relaxed">{row.gold}</td>
                    <td className="py-5 pl-4 text-xs text-gray-600 font-medium leading-relaxed">{row.diamond}</td>
                  </tr>
                ))}
              </tbody>
            </table>
