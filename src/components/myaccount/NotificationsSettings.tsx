  ...
  return (
    <div className="bg-white border border-[#D4C9A8] rounded-sm shadow-sm hover-glow transition-all duration-300 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-6 py-4 border-b border-[#F0E6D0] bg-[#F0E6D0]/20">
        ...
        <div>
          <h4 className="text-sm font-serif text-[#2C1E14]">Notification Preferences</h4>
          <p className="text-[10px] text-gray-400 font-light">Choose how and when you receive alerts.</p>
        </div>
      </div>

      <div className="p-6">
        {/* Channel Legend */}
        <div className="flex items-center gap-6 mb-5 pb-4 border-b border-[#F0E6D0]">
          <span className="text-[9px] uppercase tracking-widest font-bold text-gray-400 w-full">Notification Type</span>
          <div className="flex items-center gap-6 flex-shrink-0">
            <span className="text-[9px] uppercase tracking-widest font-bold text-gray-400 flex items-center gap-1 w-14 justify-center">
        ...
        <div className="space-y-1">
          {prefs.map((pref) => (
            <div
              key={pref.id}
              className="flex items-center gap-6 p-3 -mx-3 rounded-sm hover:bg-[#F0E6D0]/30 transition-colors"
            >
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-[#2C1E14]">{pref.title}</p>
                <p className="text-[10px] text-gray-400 font-light mt-0.5 truncate">{pref.description}</p>
              </div>
              <div className="flex items-center gap-6 flex-shrink-0">
        ...
        {/* Actions */}
        <div className="pt-5 mt-4 border-t border-[#F0E6D0] flex items-center gap-4">
          <button
            onClick={handleSave}
