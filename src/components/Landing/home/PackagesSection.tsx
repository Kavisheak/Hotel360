    ...
    <section className="w-full bg-white dark:bg-[#0A0A0A] py-24 md:py-32 px-6 md:px-12 lg:px-20 flex flex-col items-center section-reveal relative transition-colors duration-300">
      ...
      <div className="text-center mb-20 space-y-4 relative z-10">
        ...
        <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif text-[#2C1E14] dark:text-white leading-tight text-reveal stagger-2">
          Three suites of <span className="italic text-[#805D3A] dark:text-[#C9A84C] font-light">celebration.</span>
        </h2>
        <p className="text-gray-700 dark:text-gray-400 text-sm md:text-base font-light max-w-xl mx-auto pt-2 text-reveal stagger-3">
        ...
        {packages.map((pkg, index) => (
          <div 
            ...
            className={`flex flex-col overflow-hidden transition-all duration-500 relative card-entrance stagger-${index + 1} ${
              pkg.isMostLoved 
                ? 'bg-[#FDFBF7] dark:bg-[#111111] border border-[#D4C9A8] dark:border-[#C9A84C] shadow-[0_0_30px_rgba(128,93,58,0.15)] dark:shadow-[0_0_30px_rgba(201,168,76,0.15)] md:-translate-y-4 md:scale-105 z-20 py-10 px-8' 
                : 'bg-white dark:bg-[#111111] border border-[#D4C9A8] dark:border-[#C9A84C]/30 hover:border-[#805D3A]/60 dark:hover:border-[#C9A84C]/60 z-10 py-8 px-6'
            }`}
          >
            ...
            <div className="flex flex-col flex-grow text-center">
              <h3 className="text-[#805D3A] dark:text-[#C9A84C] font-serif text-2xl mb-1 mt-2">
                {pkg.name}
              </h3>
              ...
              <div className="my-6">
                ...
                <div className="flex items-center justify-center gap-2 text-gray-600 dark:text-gray-400">
                  <Users size={12} className="text-[#805D3A] dark:text-[#C9A84C]" />
                  <span className="text-[10px] uppercase tracking-widest">{pkg.guests}</span>
                </div>
              </div>

              <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed font-light mb-8 pb-8 border-b border-[#D4C9A8] dark:border-white/10">
                {pkg.description}
              </p>
              ...
              <div className="flex flex-col gap-4 text-left mb-8 flex-grow">
                {['Dedicated Wedding Planner', 'Exclusive Ballroom Access', 'Complimentary Tasting Menu', 'Valet Parking for Guests'].map((feature, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <div className="mt-1 w-1.5 h-1.5 rotate-45 bg-[#805D3A]/60 dark:bg-[#C9A84C]/60 flex-shrink-0"></div>
              ...
              <a 
                ...
                className={`w-full py-3.5 flex items-center justify-center gap-2 text-[10px] tracking-widest uppercase font-bold transition-all duration-300 ${
                  pkg.isMostLoved 
                    ? 'bg-[#D4AF37] dark:bg-[#C9A84C] text-black hover:bg-[#C9A84C] dark:hover:bg-[#B5953F]'
                    : 'bg-white dark:bg-transparent border border-[#D4C9A8] dark:border-[#C9A84C]/50 text-[#805D3A] dark:text-[#C9A84C] hover:bg-gray-50 dark:hover:bg-[#C9A84C]/10'
                }`}
              >
                Select Package
                <ArrowRight size={14} />
