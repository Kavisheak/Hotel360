...
const EventTimeline = ({ bookings = [], selectedDate = new Date() }: EventTimelineProps) => {
  ...
  const upcomingBookings = bookings
    .filter((b) => {
      const status = b.vendors?.decorator?.status;
  ...
  return (
    <div className="bg-[#F2EBE1] border border-[#E0D8C3] flex flex-col min-h-full">
        ...
        <div className="space-y-5 relative">
          {upcomingBookings.length === 0 ? (
            ...
            upcomingBookings.map((b, idx) => {
              ...
              const status = b.vendors?.decorator?.status?.toUpperCase() || 'PENDING';
      ...
      <div className="p-6 sm:p-8 pt-4 border-t border-[#E0D8C3] flex space-x-3 mt-auto">
        <Link href="/decorator/my-jobs" className="flex-1 bg-[#EBE5D9] hover:bg-[#E0D8C3] text-gray-700 py-3 font-semibold text-xs tracking-[0.15em] transition-colors text-center cursor-pointer">
          VIEW ALL
        </Link>
      </div>
    </div>
