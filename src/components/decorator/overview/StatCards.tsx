...
import { Camera, Package, CheckCircle2, Star, Loader2 } from 'lucide-react';
...
export default function StatCards() {
    ...
    const fetchStats = async () => {
      try {
        ...
        if (overviewRes.ok && overviewRes.data?.data) {
          ...
          totalEvents = d.totalBookings ?? 0;
          upcoming    = d.upcomingCount ?? 0;
          completed   = d.completedCount ?? 0;
        }

        let avgRating = 0;
        if (ratingsRes.ok && ratingsRes.data?.data?.stats) {
          avgRating = ratingsRes.data.data.stats.averageRating || 0;
        }

        setStats({ totalEvents, upcoming, completed, avgRating });
      } catch (error) {
        console.error('Error fetching decorator stats:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const cards = [
    {
      title: 'TOTAL EVENTS DECORATED',
      value: stats.totalEvents.toString(),
      sub: 'All time bookings',
      icon: <Package size={22} className="text-[#B08D2C]" />,
    },
    {
      title: 'UPCOMING SETUPS',
      value: stats.upcoming.toString(),
      sub: 'Pending or Accepted',
      icon: <Camera size={22} className="text-[#B08D2C]" />,
    },
    {
      title: 'COMPLETED PROJECTS',
      value: stats.completed.toString(),
      sub: 'Successfully delivered',
      icon: <CheckCircle2 size={22} className="text-[#B08D2C]" />,
    },
    {
      title: 'AVERAGE RATING',
      value: stats.avgRating > 0 ? stats.avgRating.toString() : 'N/A',
      sub: stats.avgRating > 0 ? '★★★★★' : 'No ratings yet',
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {cards.map((card) => (
        <div
          key={card.title}
          className="group border border-[#E0D8C3] bg-[#FDF9F1] p-6 shadow-sm hover:shadow-md hover:border-[#B08D2C] transition-all duration-300 flex items-center justify-between min-h-[110px] relative overflow-hidden"
        >
          ...
          <div className="relative z-10">
            ...
            <p className="mt-2 text-[12px] text-gray-600 font-medium">{card.sub}</p>
          </div>
