...
const BookingsGrid = ({ bookings, loading }: BookingsGridProps) => {
  ...
  let filteredBookings = bookings.filter(b => {
    ...
    const matchesStatus = statusFilter === 'all' || 
                          (statusFilter === 'pending' && b.vendors?.decorator?.status === 'Pending') ||
                          (statusFilter === 'accepted' && (b.vendors?.decorator?.status === 'Accepted' || b.vendors?.decorator?.status === 'Completed'));
  ...
  if (sortFilter === 'Earliest Date') {
    ...
    filteredBookings.sort((a, b) => (a.vendors?.decorator?.status || '').localeCompare(b.vendors?.decorator?.status || ''));
  }
        ...
        <div className="flex flex-row items-center gap-3 w-full md:w-auto">
          ...
          <div className="relative flex-1 md:flex-none">
            <select 
              ...
              className="w-full md:w-48 appearance-none bg-white border border-[#E0D8C3] px-4 py-2.5 pr-10 text-xs font-medium text-gray-700 focus:outline-none focus:border-[#B08D2C] cursor-pointer"
            >
              <option value="all">STATUS: All</option>
              <option value="pending">Awaiting Acceptance</option>
              <option value="accepted">Accepted / Setup</option>
            </select>
