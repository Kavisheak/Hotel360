...
import {
  LayoutGrid, Calendar, FolderHeart, BarChart3,
  Clock, Settings, HelpCircle, LogOut, Menu, X, BookOpen, Plus,
...
const Sidebar = () => {
  ...
  // Run only on client side to read localStorage
  useEffect(() => {
    setMounted(true);
  ...
  const bottomItems = [
    { icon: <HelpCircle size={20} />, label: 'SUPPORT', href: '/decorator/support' },
  ...
  const sidebarBody = (collapsedState: boolean) => (
    <div className="flex flex-col justify-between h-full">
      <div>
        ...
        <nav className="space-y-1">
          {navItems.map((item) => (
            <NavItem
              ...
              label={item.label}
              href={item.href}
              active={pathname === item.href || (item.href === '/decorator/bookings' && pathname?.startsWith('/decorator/bookings'))}
        ...
        >
          <Plus size={16} />
          {!collapsedState && <span>NEW EVENT</span>}
        </Link>
      </div>
      ...
      <div className="border-t border-[#E0D8C3] pt-6 space-y-4">
        {/* User Profile Block */}
        <div className={`flex items-center ${collapsedState ? 'justify-center px-0' : 'space-x-3 px-2'} py-1`} title={collapsedState ? "A. Sattar (Lead Decorator)" : undefined}>
          <img 
            src="https://images.unsplash.com/photo-1560250097-0b93528c311a?auto=format&fit=crop&w=100&h=100" 
            alt="A. Sattar Profile" 
  ...
  return (
    <>
      ...
      {/* Mobile drawer (Always fully expanded for perfect mobile view) */}
      <div
      ...
      {/* Desktop sidebar with smooth collapse animations */}
      <div 
