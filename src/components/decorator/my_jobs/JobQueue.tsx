...
const JobQueue = () => {
  ...
  // Active jobs are those not completed/cancelled by overall status, and decorator status is not Completed
  ...
  const completedJobs = bookings.filter(b => b.vendors?.decorator?.status === 'Completed' || b.status === 'Completed');
