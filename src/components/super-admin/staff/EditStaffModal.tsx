...
const EditStaffModal = ({ isOpen, onClose, onSuccess, member }: EditStaffModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await superAdminAPI.updateStaff(member.id, formData);
      if (res.ok) {
        if (onSuccess) onSuccess();
        onClose();
      } else {
        setError(res.data?.message || 'Failed to update staff account.');
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white border border-[#E0D8C3] w-full max-w-md shadow-2xl animate-fadeIn rounded-sm overflow-hidden">
        ...
        <div className="flex items-center justify-between p-5 border-b border-[#E0D8C3] bg-[#FAF6EE]">
          <h3 className="text-xl font-serif font-bold text-[#7C6A2E]">Edit Staff Member</h3>
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-4">
            <p className="text-sm text-gray-600 mb-2">
              Update details or role for this staff member.
            </p>
