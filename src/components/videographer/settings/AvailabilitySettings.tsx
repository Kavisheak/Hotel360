  ...
  return (
    <article className="bg-white border border-[#E0D8C3] p-6 sm:p-8 shadow-sm">
      ...
      <p className="text-sm text-gray-600 leading-relaxed mb-5">
        Toggle your availability for new shoot inquiries and set your preferred working schedule.
      </p>
      ...
      <div className="flex items-center justify-between border border-[#E0D8C3] bg-[#FAF6EE] px-4 py-3">
        <div>
          ...
          <p className="mt-1 text-[10px] uppercase tracking-[0.15em] text-gray-500">Selected dates and venues</p>
        </div>

        <button
          type="button"
          onClick={() => setAvailable((previous) => !previous)}
          className={`w-11 h-6 rounded-full transition-colors relative focus:outline-none ${available ? 'bg-[#7C6A2E]' : 'bg-gray-200'
            }`}
        >
          <div
            className={`w-4 h-4 rounded-full bg-white absolute top-1 transition-transform shadow-sm ${available ? 'translate-x-6' : 'translate-x-1'
              }`}
          />
        </button>
      </div>
      ...
      <div className="mt-5 space-y-3">
        <label className="block text-[10px] font-bold text-gray-400 tracking-wider uppercase">Available Days</label>
      ...
      <div className="mt-4 space-y-3">
        <label className="block text-[10px] font-bold text-gray-400 tracking-wider uppercase">Working Hours</label>
        <select className="w-full appearance-none bg-white border border-[#E0D8C3] px-4 py-2.5 text-xs font-semibold text-gray-700 focus:outline-none focus:border-[#B08D2C] cursor-pointer">
