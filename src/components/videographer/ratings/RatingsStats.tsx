...
const RatingsStats = ({ stats, loading }: RatingsStatsProps) => {
  if (loading) {
    return (
      <div className="flex justify-center items-center py-10 bg-white border border-[#E0D8C3]">
        <Loader2 className="animate-spin text-[#B08D2C]" size={24} />
      </div>
    );
  }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
      ...
      <div className="bg-white border border-[#E0D8C3] p-8 shadow-sm flex flex-col items-center justify-center text-center">
        ...
        <div className="flex space-x-1 mb-4">
          {[1, 2, 3, 4, 5].map((i) => {
            const isFilled = i <= Math.round(stats?.averageRating || 0);
            return (
              <Star 
                key={i} 
                size={16} 
                className={`${isFilled ? "text-[#B08D2C] fill-[#B08D2C]" : "text-[#E0D8C3]"}`} 
              />
            );
          })}
        </div>
        ...
        <p className="text-xs text-gray-500 font-medium italic">
          Based on {stats?.totalReviews || 0} verified client reviews
        </p>
      </div>
