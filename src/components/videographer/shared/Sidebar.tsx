...
const Sidebar = () => {
  ...
  const toggleCollapse = () => {
    ...
    localStorage.setItem('videographer-sidebar-collapsed', String(nextState));
  };

  const navItems = [
    { icon: <LayoutGrid size={20} />, label: 'OVERVIEW', href: '/videographer' },
    { icon: <CalendarDays size={20} />, label: 'MY EVENTS', href: '/videographer/my-events' },
    { icon: <Clock size={20} />, label: 'HISTORY', href: '/videographer/history' },
    { icon: <User size={20} />, label: 'PROFILE', href: '/videographer/profile' },
  ];
  ...
  const sidebarBody = (collapsedState: boolean) => (
    <div className="flex flex-col justify-between h-full">
      <div>
        ...
        <Link
          href="/videographer/my-events"
          onClick={close}
          title={collapsedState ? 'Create New Event Log' : undefined}
        ...
        >
          <Plus size={16} />
          {!collapsedState && <span>NEW ASSIGNMENT</span>}
        </Link>
      </div>
