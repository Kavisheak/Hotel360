...
const DetailMain = ({ bookingId }: DetailMainProps) => {
  ...
  const vgVendorData = booking.vendors?.videographer;
  const currentVendor = vendors.find(v => v.userId === vgVendorData?.vendorId || v.id === vgVendorData?.vendorId);
  const requestedPackageName = vgVendorData?.packageName;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#FDF9F1]">
      ...
      <div className="flex-1 px-4 sm:px-8 lg:px-10 py-6 max-w-7xl mx-auto w-full">
        ...
        {booking?.vendors?.videographer?.requirements?.specialRequests && (
          <div className="bg-[#FDE8E8] border border-[#F5D4D4] p-5 mb-8 rounded">
            ...
            <p className="text-sm italic text-gray-800">{booking.vendors.videographer.requirements.specialRequests}</p>
          </div>
        )}
      ...
      <AdvanceRequestModal
        ...
        offeredPrice={booking?.pricingBreakdown?.videographerCost || 0}
      />
          ...
          <div className="bg-white border border-[#E0D8C3] shadow-2xl p-6 max-w-2xl w-full relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-[#FAF6EE] border border-[#E0D8C3]">
                ...
                <p className="text-sm font-semibold text-gray-900">{booking?.vendors?.videographer?.packageName || 'Custom'}</p>
              </div>
              <div className="p-4 bg-[#FAF6EE] border border-[#E0D8C3]">
                <p className="text-[10px] font-bold text-[#7C6A2E] tracking-widest uppercase mb-1">Offered Price</p>
                <p className="text-sm font-semibold text-gray-900">LKR {(booking?.pricingBreakdown?.videographerCost || 0).toLocaleString()}</p>
              </div>
              ...
              {currentPackage?.coverageDuration && (
                <div className="p-4 bg-[#FAF6EE] border border-[#E0D8C3]">
                  <p className="text-[10px] font-bold text-[#7C6A2E] tracking-widest uppercase mb-1">Coverage Duration</p>
                  <p className="text-sm text-gray-700">{currentPackage.coverageDuration}</p>
                </div>
              )}
              {currentPackage?.videoServices && currentPackage.videoServices.length > 0 && (
                <div className="p-4 bg-[#FAF6EE] border border-[#E0D8C3]">
                  <p className="text-[10px] font-bold text-[#7C6A2E] tracking-widest uppercase mb-1">Video Services</p>
                  <p className="text-sm text-gray-700">{currentPackage.videoServices.join(', ')}</p>
                </div>
              )}
              {currentPackage?.deliverables && currentPackage.deliverables.length > 0 && (
                <div className="p-4 bg-[#FAF6EE] border border-[#E0D8C3] md:col-span-2">
                  <p className="text-[10px] font-bold text-[#7C6A2E] tracking-widest uppercase mb-1">Deliverables</p>
                  <p className="text-sm text-gray-700">{currentPackage.deliverables.join(', ')}</p>
                </div>
              )}
              {currentPackage?.videoQuality && (
                <div className="p-4 bg-[#FAF6EE] border border-[#E0D8C3]">
                  <p className="text-[10px] font-bold text-[#7C6A2E] tracking-widest uppercase mb-1">Video Quality</p>
                  <p className="text-sm text-gray-700">{currentPackage.videoQuality}</p>
                </div>
              )}
              {currentPackage?.cameraSetup && (
                <div className="p-4 bg-[#FAF6EE] border border-[#E0D8C3]">
                  <p className="text-[10px] font-bold text-[#7C6A2E] tracking-widest uppercase mb-1">Camera Setup</p>
                  <p className="text-sm text-gray-700">{currentPackage.cameraSetup}</p>
                </div>
              )}
