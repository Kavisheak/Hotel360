...
import { Calendar, CreditCard, FileText, Star, Loader2 } from "lucide-react";
import { videographerAPI } from "@/lib/api";

function statusClass(status: string) {
  if (status.includes("CONFIRMED") || status.includes("ACCEPTED")) return "bg-[#E6F4EA] text-[#2E7A3E] border-[#D7ECD8]";
  if (status.includes("DEPOSIT") || status.includes("COMPLETED")) return "bg-[#F7EBD6] text-[#7C6A2E] border-[#EDE3C8]";
...
export default function RecentActivity() {
  ...
  return (
    <article className="border border-[#E0D8C3] bg-[#FDF9F1] p-6 shadow-sm relative min-h-[300px]">
      {isLoading && (
      ...
      <div className="mt-5 space-y-4">
        {activities.length === 0 && !isLoading ? (
          ...
          activities.map((activity, i) => (
            <div key={`${activity.title}-${i}`} className="flex items-start gap-4 border-b border-[#E0D8C3] pb-4 last:border-b-0">
