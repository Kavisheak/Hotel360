...
export default function NewBookingMain({ onClose, onSuccess }: NewBookingMainProps) {
  ...
  const handleFinalizeBooking = async (contactInfo: any) => {
    ...
    const bookingPayload = {
      ...
      packageName: eventTypeName,
      paymentMethod: contactInfo.advancePaymentMethod || paymentMethod,
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                <p className="font-semibold text-gray-900 dark:text-gray-200 capitalize text-base">{dbPackages.find(p => (p._id || p.id) === selectedPackage)?.title || dbPackages.find(p => (p._id || p.id) === selectedPackage)?.name || selectedPackage}</p>
                ...
                {selectedPackage && (
                  <>
                    ...
                    <div className="flex items-center justify-between text-sm bg-[#FAFBF7] p-3 rounded-md border border-[#F2E5C5] mb-6">
                      <span className="font-bold text-[#1A1512] capitalize flex items-center gap-2">
                        <Gift className="w-3.5 h-3.5 text-[#C9A84C]" />
                        {dbPackages.find(p => (p._id || p.id) === selectedPackage)?.title || dbPackages.find(p => (p._id || p.id) === selectedPackage)?.name || selectedPackage}
                      </span>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="border border-[#E8DFC9] dark:border-zinc-800 rounded-xl overflow-hidden">
                          ...
                          >
                            ...
                            <span className="text-[#C69C6D] capitalize">{selectedPackage ? `${dbPackages.find(p => (p._id || p.id) === selectedPackage)?.title || dbPackages.find(p => (p._id || p.id) === selectedPackage)?.name || selectedPackage} ✓` : 'Not Selected ✗'}</span>
                          </button>
                          {mobileOpenAccordion === "package" && (
                            <div className="p-4 bg-white dark:bg-[#111111] text-xs text-gray-600 dark:text-gray-400 space-y-1 border-t border-[#E8DFC9]/40 dark:border-zinc-800/40">
                              <p><strong>Selected Package:</strong> <span className="capitalize">{selectedPackage ? `${dbPackages.find(p => (p._id || p.id) === selectedPackage)?.title || dbPackages.find(p => (p._id || p.id) === selectedPackage)?.name || selectedPackage}` : "None Selected"}</span></p>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-4 mb-4">
                  <label className="flex-1 flex items-center gap-2 p-3 border border-[#E8DFC9] dark:border-gray-800 rounded cursor-pointer hover:bg-[#FDFBF7] dark:hover:bg-zinc-800 transition-colors">
                    <input
                      type="radio"
                      name="advancePaymentMethod"
                      value="Cash"
                      checked={advancePaymentMethod === "Cash"}
                      onChange={() => setAdvancePaymentMethod("Cash")}
                      className="accent-[#C69C6D]"
                    />
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">Cash</span>
                  </label>
                  <label className="flex-1 flex items-center gap-2 p-3 border border-[#E8DFC9] dark:border-gray-800 rounded cursor-pointer hover:bg-[#FDFBF7] dark:hover:bg-zinc-800 transition-colors">
                    <input
                      type="radio"
                      name="advancePaymentMethod"
                      value="Card"
                      checked={advancePaymentMethod === "Card"}
                      onChange={() => setAdvancePaymentMethod("Card")}
                      className="accent-[#C69C6D]"
                    />
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">Card (POS)</span>
                  </label>
                </div>
              </div>
