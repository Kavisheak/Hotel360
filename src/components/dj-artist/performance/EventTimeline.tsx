...
const EventTimeline = ({ bookings = [], selectedDate = new Date() }: EventTimelineProps) => {
  ...
  // Future events excluding Declined/NotRequired (matches dashboard logic)
  ...
  return (
    <div className="bg-[#F2EBE1] border border-[#E0D8C3] flex flex-col min-h-full">
      ...
      <div className="px-6 sm:px-8 py-5 border-b border-[#E0D8C3]/50">
        <div className="space-y-4">
          {dayBookings.length === 0 ? (
            <p className="text-gray-500 text-xs italic py-2">No events scheduled for this date.</p>
          ) : (
        ...
        <div className="space-y-5 relative">
          {upcomingBookings.length === 0 ? (
            ...
            upcomingBookings.map((b, idx) => {
              ...
              const status = b.vendors?.dj?.status?.toUpperCase() || 'PENDING';
                ...
                <div key={b._id || idx} className="relative pl-8 sm:pl-10">
                  ...
                  <div className="bg-white border border-[#E0D8C3] p-4 shadow-sm">
                    ...
                    <p className="text-[10px] text-gray-500 font-serif leading-relaxed">
                      {b.vendors?.dj?.checklist?.filter((c: any) => !c.isCompleted).length || 0} tasks remaining for preparation.
                    </p>
                  </div>
                </div>
              );
      ...
      <div className="p-6 sm:p-8 pt-4 border-t border-[#E0D8C3] flex space-x-3 mt-auto">
        <Link href="/dj-artist/my-jobs" className="flex-1 bg-[#EBE5D9] hover:bg-[#E0D8C3] text-gray-700 py-3 font-semibold text-xs tracking-[0.15em] transition-colors text-center cursor-pointer">
          VIEW ALL
        </Link>
      </div>
    </div>
