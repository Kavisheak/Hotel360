...
const FAQSection = () => {
 ...
 Frequently <span className="italic text-[#C69C6D] font-light">Asked.</span>
 </h2>
 </div>
 ...
 <div 
 key={index} 
 className={`border border-[#E8DFC9] dark:border-[#C69C6D]/30 transition-all duration-300 ${isOpen ? 'bg-white dark:bg-transparent ' : 'bg-transparent hover:bg-gray-50 dark:hover:bg-[#111111]/50'}`}
 >
 <button
 onClick={() => setOpenIndex(isOpen ? null : index)}
 className="w-full px-6 py-6 flex items-center justify-between text-left"
 >
 <span className={`font-serif text-lg md:text-xl transition-colors duration-300 ${isOpen ? 'text-[#C69C6D] ' : 'text-[#2C1E14] dark:text-white'}`}>
 {faq.question}
 </span>
 <ChevronDown 
 size={20} 
 className={`text-[#C69C6D] transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}
 />
 </button>
